...
class ItemListViewModelTest {
    ...
    @Test
    fun `expect formatItemList to return sorted map of sorted item lists when there is a response from the API`() = runTest {
        assertEquals(expectedMap, viewModel.formatItemList(mockItemListMap))
    }

    @Test
    fun `expect formatItemList to return an empty map when the API responds with an empty list`() = runTest {
